...
def vivos(root_path, wavs_path, ignored_speakers = []):
    """Vivos dataset
    URL:
    ...
    file_ext = "wav"
    meta_file_train = os.path.join(root_path, 'train', 'prompts.txt')
    meta_file_test = os.path.join(root_path, 'test', 'prompts.txt')
    items = []
    
    ##Train
    with open(meta_file_train, 'r', encoding = "utf-8") as file_text:
        ...
        for line in lines:
            ...
            if os.path.exists(audio_file):
                ...
                print(f" [!] wav files don't exist - {audio_file}")
    ##Test

    with open(meta_file_test, 'r', encoding = "utf-8") as file_text:
        lines = file_text.readlines()
        for line in lines:
            file_name, text = line.split(" ", 1)
            spk, _ = file_name.split("_")
